...
class ChangeEmulator {
    ...
    useSetup = async (setup, currentSetup) => {
        let promises = []
        ...
        if (this.__config.debug) console.log("change effects:")
        if (this.__config.debug) console.log(currentSetup.collectionEffects);
        if (this.__config.debug) console.log(setup.collectionEffects);
        ...
        for (let i = 0; i < currentSetup.collectionEffects.length; ++i) {
            if (setup.collectionEffects.indexOf(currentSetup.collectionEffects[i]) < 0) {
                if (this.__config.debug) console.log("remove effect " + currentSetup.collectionEffects[i]);
        ...
        for (let i = 0; i < setup.collectionEffects.length; ++i) {
            if (currentSetup.collectionEffects.indexOf(setup.collectionEffects[i]) < 0) {
                if (this.__config.debug) console.log("add effect " + setup.collectionEffects[i]);
        ...
        for (let i = 0; i < promises.length; i++) {
            await promises[i]().catch(console.error);
        }
        return true;
    }
